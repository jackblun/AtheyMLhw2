...
df.heat$hpa <- df.heat$hpa *sd(char[,'hpa']) + mean(char[,'hpa'])
df.heat$perbush <- df.heat$perbush *sd(char[,'perbush']) + mean(char[,'perbush'])
...
ggplot(df.heat, aes(x=factor(hpa),y=factor(perbush),fill=cate,colour=cate)) + geom_tile() +
  scale_x_discrete(breaks = levels(factor(df.heat$hpa))[c(T, rep(F, 50))]) +
  scale_y_discrete(breaks = levels(factor(df.heat$perbush))[c(T, rep(F, 9))]) + 
